...
if (databaseStatistics[r.data.uuid] == null) {databaseStatistics[r.data.uuid] = {txCount:0, txBytes:0, rxCount:0, rxBytes:0, rxErrorCount:0, txErrorCount:0};} 



if (channel === 'tx') {
databaseStatistics[r.data.uuid].txCount = databaseStatistics[r.data.uuid].txCount + 1;
databaseStatistics[r.data.uuid].txBytes = databaseStatistics[r.data.uuid].txBytes + bytes;
}


if (channel === 'rx') {
databaseStatistics[r.data.uuid].rxCount = databaseStatistics[r.data.uuid].rxCount + 1;
databaseStatistics[r.data.uuid].rxBytes = databaseStatistics[r.data.uuid].rxBytes + bytes;
}
